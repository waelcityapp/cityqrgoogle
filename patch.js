...
code = code.replace(
  /const { error: err1 } = await client\s*\.from\('profiles'\)\s*\.update\({([^}]*)}\)\s*\.eq\('email', user\.email\);\s*if \(!err1\) updated = true;/g,
...
code = code.replace(
  /const { error: err3 } = await client\s*\.from\('profiles'\)\s*\.update\({([^}]*)}\)\s*\.eq\('id', user\.id\);\s*if \(!err3\) updated = true;/g,
